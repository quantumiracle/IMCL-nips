...
def NLLLoss_sampled(logits, num_act, numSamples = 1):
    logits_expanded = tf.tile(logits, [numSamples, 1])
    noise_shape = tf.concat([tf.shape(logits), [numSamples,]],0)
    ...
    onehot_labels = tf.sparse_to_dense(concated, tf.stack([batch_size, num_act]), 1.0, 0.0)
...
def build_train(make_obs_ph, q_func, num_actions, optimizer, isKfac=False, fisher_metric='gn', grad_norm_clipping=None, gamma=1.0,
...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if isKfac:
            #Fisher loss
            if fisher_metric == 'gn':
                sample_net = q_t_selected+ tf.random_normal(tf.shape(q_t_selected))
                qf_fisher_loss = -tf.reduce_mean(tf.pow(q_t_selected - tf.stop_gradient(sample_net), 2))
            else:
                qf_fisher_loss = NLLLoss_sampled(logits=q_t, num_act=num_actions)

            # compute optimization op (potentially with gradient clipping)
            grads = optimizer.compute_gradients(weighted_error, var_list=q_func_vars)
            optimizer.compute_and_apply_stats(qf_fisher_loss, var_list=q_func_vars)
            optimize_expr, q_runner = optimizer.apply_gradients(grads)
        else:
        ...
        if isKfac:
            return act_f, train, update_target, {'q_values': q_values}, q_runner
        else:
            return act_f, train, update_target, {'q_values': q_values}, None
