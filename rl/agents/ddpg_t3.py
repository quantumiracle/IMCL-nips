...
class DDPGAgent_t3(object):
    ...
    def generate_train_op(self):
        ...
        def learn(sd, ad, rd, isdoned, s_d):
            self.sess.run(critic_train, feed_dict={S:sd, S_:s_d, isdone:isdoned, A:ad, R:rd})
            self.sess.run(actor_train, feed_dict={S:sd})

            if self.t_replace_counter % self.t_replace_iter == 0:
                self.sess.run([update_critic_target, update_actor_target],
                    feed_dict={tau:1e-3})
    ...
    def forward(self, obs):
        ...
        state = np.reshape(obs, (1,len(obs)))
        ...
        #     noise_added = noise_sample * self.action_multiplier
        #     action += noise_added
    ...
    def backward(self, epi_selecting):
        if self.memory.size() > self.batch_size*16:
            for _ in xrange(self.train_multiplier):
                ...
                [state1, action, reward, isdone, state2] = self.memory.sample_batch(self.batch_size)
                
                self.learn(state1, action, reward, isdone, state2)
    ...
    def play(self, env, epi_index, episode_max_steps, noise_level, epi_sel, act_repetition=1):
        ...
        while episode_step < episode_max_steps:
            ...
            episode_memory.append((obs_before_act, action, reward, isdone, obs))
