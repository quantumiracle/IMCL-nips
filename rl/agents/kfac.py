...
class KfacOptimizer():
    ...
    def getFactors(self, g, varlist):
        ...
        def searchFactors(gradient, graph):
            ...
            if 'AddN' in bpropOp_name:
                ...
                fpropOp_name = re.search(   #re: regularization search
                ...
                if fpropOp.op_def.name in KFAC_OPS:
                    ...
                    if len(bInputsList) > 0:
                        ...
                        if len(bTensor.get_shape()) > 0 and bTensor.get_shape()[0].value == None:
                            ...
                            #print(bTensorShape)
                            #print(bTensor.shape)
                            #bTensor=bTensor.reshape((1,5,1,8))
                            #bTensor.set_shape(bTensorShape)
                        #bTensors.append(bTensor)
                        '''
                    ...
                    def optimOp():
                        def updateOptimOp():
                            if self._full_stats_init:
                                ...
                                return optim.apply_gradients(list(zip(u, varlist)))   #!!key step to update weights
                        if self._full_stats_init:
    ...
    def apply_gradients(self, grads):
        ...
        def coldSGDstart():
            ...
            coldOptim_op = coldOptim.apply_gradients(sgd_grads)    #!!!!cold update
            if KFAC_DEBUG:
