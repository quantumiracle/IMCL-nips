...
from rl.processor import transform_observation
from math import *
...
class Actor(object):
    def __init__(self, sess, S, S_, A, act_dims, learning_rate, t_replace_iter=1):
        self.sess = sess
        self.S = S
        self.S_ = S_
        self.A = A
    ...
    def _build_net(self, actor_input, scope):
        with tf.variable_scope(scope):
            ...
            actor = tl.layers.DenseLayer(actor, n_units=160, name='dense_layer1',
                                        act= lambda x : tl.act.lrelu(x, 0.2))
            actor = tl.layers.ReshapeLayer(actor, [-1, 160, 1], name ='reshape_layer')     
            actor = tl.layers.Conv1dLayer(actor, shape=[5, 1, 8], name='conv_layer1',
                                        act= lambda x : tl.act.lrelu(x, 0.2))
            actor = tl.layers.Conv1dLayer(actor, shape=[3, 8, 4], name='conv_layer2',
                                        act= lambda x : tl.act.lrelu(x, 0.2))
            actor = tl.layers.Conv1dLayer(actor, shape=[3, 4, 2], name='conv_layer3',
                                        act= lambda x : tl.act.lrelu(x, 0.2))
            actor = tl.layers.FlattenLayer(actor,name ='flatten_layer')
            actor = tl.layers.DenseLayer(actor, n_units=160, name='dense_layer2',
                                        act= lambda x : tl.act.lrelu(x, 0.2))
            actor = tl.layers.DenseLayer(actor, n_units=80, name='dense_layer3',
                                        act= lambda x : tl.act.lrelu(x, 0.2))
...
class Critic(object):
    ...
    def _build_net(self, critic_obs_input, critic_act_input, scope):
        with tf.variable_scope(scope):  
            ...
            critic = tl.layers.DenseLayer(critic, n_units=160, name='dense_layer1',
                                        act= lambda x : tl.act.lrelu(x, 0.2))
            critic = tl.layers.ReshapeLayer(critic, [-1, 160, 1], name ='reshape_layer')
            critic = tl.layers.Conv1dLayer(critic, shape=[5, 1, 8], name='conv_layer1',
                                        act= lambda x : tl.act.lrelu(x, 0.2))
            critic = tl.layers.Conv1dLayer(critic, shape=[3, 8, 4], name='conv_layer2',
                                        act= lambda x : tl.act.lrelu(x, 0.2))
            critic = tl.layers.Conv1dLayer(critic, shape=[3, 4, 2], name='conv_layer3',
                                        act= lambda x : tl.act.lrelu(x, 0.2))
            critic = tl.layers.FlattenLayer(critic,name ='flatten_layer')
            critic = tl.layers.DenseLayer(critic, n_units=160, name='dense_layer2',
                                        act= lambda x : tl.act.lrelu(x, 0.2))
            critic = tl.layers.DenseLayer(critic, n_units=80, name='dense_layer3',
                                        act= lambda x : tl.act.lrelu(x, 0.2))
...
class DDPGAgent(object):
    def __init__(self, memory, noise, observation_dims, action_dims, batch_size, nb_max_episodes, gamma=.99):
        ...
        # Networks
        self.actor = Actor(self.sess, self.S, self.S_, self.A, 
                        self.action_dims, self.actor_learning_rate)
        self.critic = Critic(self.sess, self.S, self.S_, self.R, self.A, self.actor.a_, 
    ...
    def backward(self):
        if self.memory.nb_entries > self.batch_size*1:
    ...
    # play one episode
    def play(self, env, epi_index, episode_max_steps, act_repetition=1):
        timer = time.time()
        episode_step = 0
        episode_reward = 0.
        episode_memory = []
